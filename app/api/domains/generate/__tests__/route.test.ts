...
>;
const mockCheckAvailabilityNamecom =
  checkAvailabilityNamecom as jest.MockedFunction<
    typeof checkAvailabilityNamecom
  >;
    ...
    it("should generate domain suggestions successfully", async () => {
      ...
      expect(mockOpenRouterChat).toHaveBeenCalledWith({
        messages: [
          {
            role: "system",
            content: expect.stringContaining("Generate domain name ideas"),
          },
  ...
  describe("input validation", () => {
    ...
    it("should reject malformed JSON", async () => {
      const request = createMockNextRequest(
        "http://localhost:3000/api/domains/generate",
        {
          method: "POST",
