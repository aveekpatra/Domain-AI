...
export async function POST(req: NextRequest) {
  ...
  if (!corsAllowed(origin)) {
    if (process.env.NODE_ENV !== "production")
      console.warn("[improve] blocked by CORS", { origin });
  ...
  if (!limited.ok) {
    if (process.env.NODE_ENV !== "production")
      console.warn("[improve] rate limited", {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production")
      console.log("[improve] input ok", { len: input.prompt.length });
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : String(e);
    if (process.env.NODE_ENV !== "production")
      console.error("[improve] invalid input", msg);
  ...
  try {
    if (process.env.NODE_ENV !== "production")
      console.log("[improve] calling openrouter", {
    ...
    if (process.env.NODE_ENV !== "production")
      console.log("[improve] openrouter ok", { length: text.length });
